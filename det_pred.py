...
def mult2single(bounds):
    x_range = [x[0] for x in sum(bounds,[])]
    y_range = [x[1] for x in sum(bounds,[])]
    return bound2box(min(x_range),max(x_range),min(y_range),max(y_range))

def mult2point(bounds):
    x_range = [x[0] for x in sum(bounds,[])]
    y_range = [x[1] for x in sum(bounds,[])]
...
class TextDetector(object):
    def __init__(self,args,config_path):
        ...
        self.det_algorithm = self.config['Architecture']['algorithm']
        pre_process_list = [{
            'DetResizeForTest': {
